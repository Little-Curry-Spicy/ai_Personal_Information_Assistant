...
- 首次进入对话区时展示助手欢迎语与若干「预备问题」按钮，一键发送对应提问（文案见 `front/src/App.tsx` 中 `CHAT_STARTERS`：工作经历 / 技术栈 / 代表性项目 / GitHub 方向；后端已对齐「先向量库、必要时再拉 GitHub 用户仓库列表」的策略）
- 支持追问、连续对话
...
5. **预备问题与简历优先**：问句里若含「工作职责」「量化成果」等词，不应误判成「项目类」而取消简历优先；后端已对「工作经历 / 每段工作」等模式强制走简历优先检索。
6. **某一 Qdrant 集合不可用**：若仅 `github` 集合创建失败，检索仍会尽量使用 `resume` 集合，避免因一侧故障导致整条检索报错。
