    ...
    <div className="tool-panel tool-panel--web-search">
      <div className="tool-panel__head">
        <span className="tool-panel__label">
          <Search className="tool-panel__label-icon ui-icon" size={ICON_SM} strokeWidth={2} aria-hidden />
          网页搜索
        </span>
        {query ? (
          <span className="tool-panel__query">
            「{query}」
            {count != null ? (
              <span className="tool-panel__count"> · 约 {count} 条</span>
            ) : null}
          </span>
        ) : null}
      </div>
        ...
        <ul className="tool-panel__results">
          {items.map((it, idx) => (
            <li key={`${it.ref}-${idx}`} className="tool-panel__result">
              {it.ref ? (
                <span className="tool-panel__ref">来源 {it.ref}</span>
              ) : null}
              {it.title ? (
    ...
    >
      <div className="tool-panel__head tool-panel__head--send-mail">
        <span className="tool-panel__label tool-panel__label--send-mail">
          <Mail className="tool-panel__label-icon ui-icon" size={ICON_SM} strokeWidth={2} aria-hidden />
          发送邮件
        </span>
        {subject ? (
      ...
      {bodyText || bodyHtml ? (
        ...
        <div className="tool-panel__mail-body-block">
          ...
          <div className="tool-panel__mail-body-scroll tool-panel__mail-body-scroll--empty">
            <span className="tool-panel__mail-body-placeholder">正文将随后显示…</span>
            <span className="tool-panel__mail-cursor tool-panel__mail-cursor--block" aria-hidden />
          </div>
        </div>
      ) : null}
      {inProgress ? (
        <div className="tool-panel__mail-status tool-panel__mail-status--progress" role="status">
          {progress === 'input-streaming' ? '正在补全邮件内容与参数…' : '正在投递邮件…'}
        </div>
      ) : output.trim() ? (
...
function ToolPendingPanel({ name, hint, paused = false }: { name: string; hint?: string; paused?: boolean }) {
  ...
  return (
    ...
    >
      <span className="tool-panel__pending-text">
        {paused ? (
          <AlertCircle className="tool-panel__pending-icon ui-icon" size={ICON_SM} strokeWidth={2} aria-hidden />
        ) : (
          <Loader2
            className="tool-panel__pending-icon ui-icon ui-icon--spin"
            size={ICON_SM}
            strokeWidth={2}
            aria-hidden
          />
        ...
        {paused ? (
          <>
            已停止「<strong>{label}</strong>」
          </>
        ) : (
          <>
            正在执行「<strong>{label}</strong>」
            {hint ? <>：{hint}</> : '…'}
          </>
        )}
      </span>
...
function ToolErrorPanel({ name, message }: { name: string; message: string }) {
  ...
  return (
    <div className="tool-panel tool-panel--error" role="alert">
      <span className="tool-panel__label tool-panel__label--error">
        <AlertCircle className="tool-panel__label-icon ui-icon" size={ICON_SM} strokeWidth={2} aria-hidden />
        {label}
      </span>
...
function DefaultToolOutput({ value }: { value: JsonValue | WebSearchToolOutput }) {
  ...
  return (
    <pre className="tool-panel tool-panel--default">
      <span className="tool-panel__default-hint">
        <Wrench className="tool-panel__label-icon ui-icon" size={ICON_SM} strokeWidth={2} aria-hidden />
        工具原始输出
      </span>
      {text}
    </pre>
...
function ToolMessagePart({ part, paused = false }: { part: AnyToolPart; paused?: boolean }) {
  ...
  if (part.state !== 'output-available') {
    if (paused) {
      return <ToolPendingPanel name={name} paused />
    }
  ...
  if (isToolUIPart(part)) {
    return (
      <div className="chat-tool-wrap">
        <ToolMessagePart part={part} paused={paused} />
      </div>
    )
  }
  return null
